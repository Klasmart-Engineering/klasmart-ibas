...
def invoice_callback(callback_data):
    ...
    if invoice_status == "PAID":
        from kidsloop_indonesia_b2c_automation.strapi_gateway import api_requests, models
        paid_id = settings.STRAPI_STATUS_PAID_ID
        external_id = callback_data["external_id"]
        schedule_id = models.ScheduleInvoce.objects.get(external_id=external_id).schedule_id
